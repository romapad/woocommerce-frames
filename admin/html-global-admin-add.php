<div class="wrap woocommerce">
	...
	<form method="POST" action="">
		<table class="form-table global-addons-form meta-box-sortables">
			...
			<tr>
				<th>
					<label for="addon-priority"><?php _e( 'Priority', 'grmpd-frames' ); ?></label>
				</th>
				<td>
			...
			<tr>
				<th>
					<label for="addon-objects"><?php _e( 'Applied to...', 'grmpd-frames' ); ?></label>
				</th>
				<td>
					<select id="addon-objects" name="addon-objects[]" multiple="multiple" style="width:50%;" data-placeholder="<?php _e('Choose some options&hellip;', 'grmpd-frames'); ?>" class="chosen_select wc-enhanced-select">
						<option value="0" <?php selected( in_array( '0', $objects ), true ); ?>><?php _e( 'All Products', 'grmpd-frames' ); ?></option>
						<optgroup label="<?php _e( 'Product category notifications', 'grmpd-frames' ); ?>">
							<?php
								$terms = get_terms( 'product_cat', array( 'hide_empty' => 0 ) );

								foreach ( $terms as $term ) {
									echo '<option value="' . $term->term_id . '" ' . selected( in_array( $term->term_id, $objects ), true, false ) . '>' . __( 'Category:', 'grmpd-frames' ) . ' ' . $term->name . '</option>';
								}
							?>
						</optgroup>
